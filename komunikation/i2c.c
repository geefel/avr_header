...
#define I2CDATA PINDEF(C, 4)
#define I2CCLOCK PINDEF(C, 5)
...
void sendDataI2C(uint8_t deviceAddress, uint8_t registerStartNr, uint8_t *data, uint8_t numOfData) {
	start_I2C();
	send_I2C_Address(deviceAddress, 0);
	send_I2C_Data(registerStartNr);
...
void getDataI2C(uint8_t deviceAddress, uint8_t registerStartNr, uint8_t *data, uint8_t numOfData) {
	...
	start_I2C();
	send_I2C_Address(deviceAddress, 0);
	send_I2C_Data(registerStartNr);					//Registerplatz von dem gelesen werden soll angeben
	//stop_I2C();														//brauchen wir hier nicht, weil es weiter geht
	start_I2C();
	send_I2C_Address(deviceAddress, 1);
	for (i = 0; i < numOfData - 1; ++i)
		data[i] = read_I2C_Data_ACK();
	data[++i] = read_I2C_Data_NACK();				//Daten holen MUSS bei letztem Byte mit NACK erfolgen 
	stop_I2C();
}
