...
void out(char *txt, ...) {
  ...
  int n;
  char back[10];
	char *ph = back;	//ph bekommt die Startadresse von back und zeigt damit auf das erste Element von back
	int h;
      ...
      case 'd':
        n = (int)va_arg(args, int);
				...
				while(n) { 								 	//Ziffern rueckwaerts nach back schreiben
					...
					*(ph++) = (char)('0' + h); 	//an ph übergeben (Ascii 0 + int h = Ascii von Ziffer!!!!). Die Ziffern sind in umgekehrter Reihenfolge in ph! 
				}
				while(ph > back) 	//Nun Ziffern ausgeben, ph ist Zeiger auf Elemente von back und wird herunter gezählt bis ph-Adresse = back[0]-Adresse
					sendDat(*(--ph));
        break;
      case 0:
        return;
